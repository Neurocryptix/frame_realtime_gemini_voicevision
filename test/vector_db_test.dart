...
void objectBoxTest(String description, dynamic Function() body,
    {required bool objectBoxAvailable}) {
  test(description, () {
  ...
  group('VectorDbService MobileBERT Integration Tests', () {
    ...
    setUpAll(() async {
      try {
        ...
        // ObjectBox not available in test environment: $e
        // All tests in this group will be skipped
    ...
    setUp(() async {
      ...
      if (objectBoxAvailable) {
        ...
        vectorDb = null; // Don't create any service when not available
      }
    });

    tearDown(() async {
      if (objectBoxAvailable && vectorDb != null) {
    ...
    test('Embedding generation (384 dimensions)', () async {
      ...
      final allValuesInRange =
          embedding.every((value) => value >= -2.0 && value <= 2.0);
    ...
    test('Vector similarity search functionality', () async {
      ...
      final testDocs = [
        ...
        {
          'content': 'Frame glasses provide augmented reality',
          'type': 'glasses'
        },
      ...
      expect(results, isNotEmpty, reason: 'Should find similar documents');
    ...
    test('Sample data functionality', () async {
      ...
      expect(count, greaterThan(0), reason: 'Should add sample documents');

      final docs = await vectorDb!.getAllDocuments();
      final hasSampleDocs = docs.any((doc) =>
          doc.metadata != null && doc.metadata!.contains('type=sample'));

      expect(hasSampleDocs, true, reason: 'Should contain sample documents');
    });
    ...
    test('Model testing functionality', () async {
      ...
      expect(hasTestLogs, true, reason: 'Should attempt model testing');
    });
