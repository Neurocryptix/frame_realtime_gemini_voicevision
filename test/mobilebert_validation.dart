  ...
  group('MobileBERT Model Validation', () {
    ...
    test('Asset files exist and are accessible', () async {
      ...
      try {
        ...
        expect(modelData.lengthInBytes,
            greaterThan(1000000)); // Should be several MB
        debugPrint(
            '✅ Model file size: ${(modelData.lengthInBytes / 1024 / 1024).toStringAsFixed(1)} MB');
      } catch (e) {
      ...
      try {
        ...
        final lines = vocabContent
            .split('\n')
            .where((line) => line.trim().isNotEmpty)
            .toList();
        expect(lines.length,
            greaterThan(30000)); // MobileBERT vocab should be ~30k tokens
      ...
      for (int i = 0; i < inputTensors.length; i++) {
        ...
        if (tensor.shape.length == 2) {
          expect(tensor.shape[1], equals(maxSequenceLength),
              reason:
                  'Input tensor should have sequence length of $maxSequenceLength');
        }
      }
      ...
      for (int i = 0; i < outputTensors.length; i++) {
        ...
        if (tensor.shape.length == 2) {
          expect(tensor.shape[1], equals(embeddingSize),
              reason:
                  'Output tensor should have embedding size of $embeddingSize');
        }
      }
    ...
    test('TensorUtils helper functions work correctly', () {
      ...
      final shape = TensorUtils.getTensorShape([
        [
          [1, 2],
          [3, 4]
        ],
    ...
    test('MobileBERT embedding generation (if model loads)', () async {
      ...
      try {
        ...
        final allValuesInRange =
            embedding.every((value) => value >= -5.0 && value <= 5.0);
        ...
        debugPrint(
            '  Sample values: [${embedding.take(5).map((v) => v.toStringAsFixed(4)).join(", ")}...]');

        // Calculate and debugPrint embedding statistics
        final mean =
            embedding.fold(0.0, (sum, val) => sum + val) / embedding.length;
        final variance = embedding.fold(
                0.0, (sum, val) => sum + (val - mean) * (val - mean)) /
            embedding.length;
        final norm = embedding.fold(0.0, (sum, val) => sum + val * val);

        debugPrint(
            '  Statistics: mean=${mean.toStringAsFixed(4)}, variance=${variance.toStringAsFixed(4)}, norm=${norm.toStringAsFixed(4)}');
      } catch (e) {
        ...
        debugPrint(
            '⚠️ This may be due to platform constraints in test environment');
      }
    });
  });
}
