...
class FrameGeminiAudioIntegration {
  ...
  Future<void> _simulateSpeechToText(
      Uint8List audioData, double duration) async {
  ...
  Future<void> _sendToGemini(String transcript) async {
    try {
      ...
      final response =
          await _geminiModel.generateContent([Content.text(transcript)]);
  ...
  Future<void> _displayOnFrame(String text) async {
    ...
    final displayText = text.length > 50 ? '${text.substring(0, 47)}...' : text;
