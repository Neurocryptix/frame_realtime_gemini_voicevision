...
class StreamObserver<T> {
  ...
  void observe(Stream<T> originalStream, {String? streamName}) {
    if (_isActive) {
      _logger?.call(
          '⚠️ StreamObserver already active for ${streamName ?? "unknown"}');
      return;
    }

    _logger?.call(
        '👁️ Starting stream observation for ${streamName ?? "unknown"}');
    _isActive = true;
    ...
    _subscription = originalStream.listen(
      ...
      onError: (error) {
        _logger?.call(
            '❌ Stream observation error for ${streamName ?? "unknown"}: $error');
        _controller.addError(error);
      },
      onDone: () {
        _logger?.call(
            '✅ Stream observation completed for ${streamName ?? "unknown"}');
        _isActive = false;
      },
    );
  }
  ...
  void stopObserving({String? streamName}) {
    ...
    _logger
        ?.call('⏹️ Stopping stream observation for ${streamName ?? "unknown"}');
    _isActive = false;
...
class TimestampedData<T> {
  ...
  @override
  String toString() =>
      'TimestampedData(timestamp: $timestamp, dataType: ${T.runtimeType})';
}
  ...
  void observe(Stream<Uint8List> originalStream, {String? streamName}) {
    ...
    observedStream.listen((timestampedAudio) {
      ...
      if (_totalPacketsObserved % 100 == 0) {
        ...
        _logger?.call(
            '📊 Audio observed: $_totalPacketsObserved packets, ${kbObserved}KB');
      }
    });
  }
...
class PhotoStreamObserver extends StreamObserver<Uint8List> {
  ...
  void observe(Stream<Uint8List> originalStream, {String? streamName}) {
    ...
    observedStream.listen((timestampedPhoto) {
      ...
      final photoSizeKb =
          (timestampedPhoto.data.length / 1024).toStringAsFixed(1);
      _logger?.call(
          '📸 Photo observed: ${photoSizeKb}KB at ${timestampedPhoto.timestamp}');
    });
  }

  /// Get photos captured within a time window
  List<TimestampedData<Uint8List>> getPhotosInWindow(
      DateTime centerTime, Duration window) {
    return _recentPhotos
        .where((photo) => photo.isWithinWindow(centerTime, window))
        .toList();
  }
