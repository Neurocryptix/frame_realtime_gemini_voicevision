...
class AgentCore {
  ...
  Future<void> _processAudio(
      TimestampedData<Uint8List> timestampedAudio) async {
    if (!_isEnabled || _isProcessing) return;

    try {
      // Run ASR on the audio data (non-blocking)
      final asrResult =
          await _asrService.transcribeAudio(timestampedAudio.data);
      ...
      if (asrResult != null && asrResult.text.trim().isNotEmpty) {
        ...
        final output = AgentOutput(
          ...
          associatedImageTimestamps:
              correlatedImages.map((img) => img.timestamp).toList(),
          metadata: {
            'audioLength': timestampedAudio.data.length,
            'processingTime': DateTime.now()
                .difference(timestampedAudio.timestamp)
                .inMilliseconds,
          },
        );
        ...
        _outputs.add(output);
        _logger(
            '🗣️ ASR: "${asrResult.text}" (${correlatedImages.length} images)');
  ...
  Future<void> _processPhoto(
      TimestampedData<Uint8List> timestampedPhoto) async {
      ...
      if (ocrResult != null && ocrResult.text.trim().isNotEmpty) {
        ...
        final output = AgentOutput(
          ...
          metadata: {
            ...
            'processingTime': DateTime.now()
                .difference(timestampedPhoto.timestamp)
                .inMilliseconds,
          },
        );
    ...
    try {
      ...
      if (llmResponse != null) {
        ...
        if (llmResponse.content.trim().isNotEmpty) {
          ...
          _logger(
              '🧠 LLM: "${llmResponse.content}" (${llmResponse.toolCalls.length} tools)');
        }
      }
    } catch (e) {
  ...
  String _buildLLMContext(
      AgentOutput output, List<TimestampedData<Uint8List>> correlatedImages) {
  ...
  Future<void> _executeTool(
      ToolCall toolCall, AgentOutput originalOutput) async {
    try {
    ...
    try {
      ...
      if (_sessionStartTime != null) {
        ...
        final correlationReport = _timestampManager.generateCorrelationReport(
          asrTimestamps: _outputs
              .where((o) => o.type == AgentOutputType.asr)
              .map((o) => o.timestamp)
              .toList(),
          ocrTimestamps: _outputs
              .where((o) => o.type == AgentOutputType.ocr)
              .map((o) => o.timestamp)
              .toList(),
          availablePhotos: _recentImages,
        );
