...
class TensorUtils {
  ...
  static List<List<double>> reshape2DFloat32(
      Float32List data, int rows, int cols) {
    if (data.length != rows * cols) {
      throw ArgumentError(
          'Data length ${data.length} does not match shape [$rows, $cols]');
    }
  ...
  static List<List<List<T>>> reshape3D<T>(
      List<T> data, int dim1, int dim2, int dim3) {
    if (data.length != dim1 * dim2 * dim3) {
      throw ArgumentError(
          'Data length ${data.length} does not match shape [$dim1, $dim2, $dim3]');
    }
